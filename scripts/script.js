$(document).ready(function() {
    ...
    $('#contenedor_redes_footer img').each(function(index) {
        ...
        var enlace = $('<a>', { href: redesSociales[index].enlace, target: '_blank' });
        $(this).wrap(enlace); // Envolver la imagen con el enlace
    });

    $('#Contenedor_redes_mh .redes_mh').each(function(index) {
        // Agregar enlace a cada imagen
        var enlace = $('<a>', { href: redesSocialesMH[index].enlace, target: '_blank' });
        $(this).wrap(enlace); // Envolver la imagen con el enlace
    });

    var enlaceReserva = $('<a>', { href: 'reserva.html', target: '_blank' });
    $('#Boton_reserva_mh').wrap(enlaceReserva);

    var linkPolitica = $('<a>', { href: 'https://www.privacypolicies.com/live/6cc6bb19-5b51-4de4-a028-86e7dd0290ac', target: '_blank' });
